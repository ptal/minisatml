...
int main(int argc, char** argv)
{
    ...
    if (false) reportf("This is MiniSat 2.0 beta\n");
#if defined(__linux__)
    fpu_control_t oldcw, newcw;
    _FPU_GETCW(oldcw); newcw = (oldcw & ~_FPU_EXTENDED) | _FPU_DOUBLE; _FPU_SETCW(newcw);
    if (false) reportf("WARNING: for repeatability, setting FPU to use double precision\n");
#endif
    double cpu_time = cpuTime();
    ...
    if (false) reportf("|  Parsing time:         %-12.2f s                                       |\n", parse_time);
